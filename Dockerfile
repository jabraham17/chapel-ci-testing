...
RUN apt-get update -y && \
    apt-get install -y gcc g++ m4 perl python3 python3-dev python3-venv bash make mawk git pkg-config cmake && \
...
RUN export PATH=/opt/chapel/bin/$($CHPL_HOME/util/chplenv/chpl_bin_subdir.py):$PATH
RUN make check
